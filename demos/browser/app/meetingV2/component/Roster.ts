...
class Attendee {
  ...
  constructor(id: string, name: string) {
    this.id = id;
    this.name = name;
    this.audioStatus = 'SendReceive';
    this.signalStrength = 1;
    this.isContentAttendee = new DefaultModality(id).hasModality(DefaultModality.MODALITY_CONTENT);
  }
}
...
export default class Roster {
  ...
  addAttendee(attendeeId: string, attendeeName: string, allowAttendeeCapabilities: boolean): void {
    ...
    const attendeeStatusElement: HTMLSpanElement = document.createElement(
      'span'
    ) as HTMLSpanElement;
    ...
    attendeeCheckbox.className =
      'roster-checkbox form-check-input m-0 flex-shrink-0 ' +
      (attendee.isContentAttendee ? ' invisible' : '');
  ...
  removeAttendee(attendeeId: string): boolean {
    ...
    const childToDelete = document.getElementById(
      Roster.ATTENDEE_ELEMENT_PREFIX + attendeeId
    ) as HTMLLIElement;
  ...
  /**
   * Sets the audio status of the attendee. This helps whether they are permanently muted.
   * @param attendeeId - the attendeeId for whom we intend to set the audio signal strength.
   * @param audioStatus - value indicating the audio status.
   */
  setAudioStaus(attendeeId: string, audioStatus: string): void {
    if (!this.hasAttendee(attendeeId)) {
      return;
    }
    const attendee: Attendee = this.attendeeInfoMap.get(attendeeId);
    attendee.audioStatus = audioStatus;
    this.handleRosterStatusUpdate(attendeeId);
  }
  ...
  unselectAll() {
    ...
    this.updateRosterMenu();
    for (const checkboxElement of Array.from(
      document.getElementsByClassName('roster-checkbox')
    ) as HTMLInputElement[]) {
  ...
  private handleRosterStatusUpdate(attendeeId: string): void {
    ...
    } else if (attendee.audioStatus === 'Receive') {
      statusText = 'Perm MUTED';
      statusClass += 'bg-danger';
    ...
    const attendeeElement: HTMLLIElement = document.getElementById(
      Roster.ATTENDEE_ELEMENT_PREFIX + attendeeId
    ) as HTMLLIElement;
    const attendeeStatusElement: HTMLSpanElement = attendeeElement.getElementsByClassName(
      'status'
